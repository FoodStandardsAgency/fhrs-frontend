...
import businessNameToUrl from "./business";
...
const api = {
        ...
        if(type === 'establishments' && Object.keys(parameters).length !== 0) {
          ...
          Object.entries(parameters).forEach(param => {
            if (param[1] && !(param[1] === 'all' || param[1] === '-1')) {
              paramArray.push(`${param[0]}=${encodeURIComponent(param[1])}`);
            }
          });
  ...
  /**
   *  Return the matching authority from the FriendlyName
   *  @param id the FriendlyName of a local authority
   *  @return object authority details
   */
  getAuthorityFromName(id) {
    return new Promise((resolve, reject) => {
      this.fetchClient().get(`authorities`).then((value) => {
        const authority = value.data.authorities.filter( authority => {
          return authority.FriendlyName === id;
        });
        resolve(authority[0]);
      }).catch((reason) => {
        reject(reason);
